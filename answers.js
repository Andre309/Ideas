// Масиви для кожної кнопки
const genres = [
  'Комедія',
  'Драма',
  'Фантастика',
  'Трилер',
  'Детектив',
  'Еротика',
  'Жахи',
  'Казка',
  'Містика',
  'Пригоди',
  'Психологічна проза',
  'Романтика',
  'Трагедія',
  'Байка',
  'Гумор',
  'Легенда',
  'Міф',
  'Наукова фантастика',
  'Памфлет',
  'Постапокаліптика',
  'Притча',
  'Сатира',
  'Трагікомедія',
  'Утопія',
  'Антиутопія'
];

const locations = [
  'Ліс',
  'Місто',
  'Космос',
  'Школа',
  'Війна',
  'Вулиця',
  'Дитинство',
  'Апокаліптичний світ',
  'Закинутий будинок',
  'Кінотеатр',
  'Кладовище',
  'Концерт',
  'В ліжку',
  'Лікарня',
  'Літак',
  'Метро',
  'На небесах',
  'Озеро',
  'Офіс',
  'Парк',
  'Пекло',
  'Пісочниця',
  'Потяг',
  'Ресторан',
  'Село',
  'На смертному одрі',
  'Тюрма',
  'Університет',
  'Церква',
  'Магазин',
  'У підвалі',
  'Океан',
  'Роддом',
  'Шафа',
  'Сміттєзвалище',
  'Острів',
  'Полон',
  'Вдома',
  'В гостях',
  'У майбутньому',
  'У минулому',
  'Гори',
  'Пустеля',
  'Печера',
  'Водоспад',
  'Джунглі',
  'Степ',
  'Болото',
  'Річка',
  'Пляж',
  'Тундра',
  'Замок',
  'Маяк',
  'Готель',
  'Гуртожиток',
  'Аеропорт',
  'Вокзал',
  'Порт',
  'Кафе',
  'Бібліотека',
  'Музей',
  'Театр',
  'Спортзал',
  'Стадіон',
  'Басейн',
  'Нічний клуб',
  'Казино',
  'Ярмарок',
  'Цирк',
  'Зоопарк',
  'Ботанічний сад',
  'Ферма',
  'Шахта',
  'Завод',
  'Будівельний майданчик',
  'Поліцейська дільниця',
  'Суд',
  'Банк',
  'Аптека',
  'Дитячий садок',
  'Дитячий будинок',
  'Психіатрична лікарня',
  'Обсерваторія',
  'Піраміда',
  'Катакомби',
  'Бункер',
  'Підводний човен',
  'Космічна станція',
  'Інша планета',
  'Паралельний світ',
  'Сновидіння',
  'Віртуальна реальність',
  'Кухня',
  'Горище',
  'Дах будинку',
  'Сад',
  'Гараж',
  'Автобус',
  'Ринок',
  'Санаторій'
];

const situations = [
  'Втеча',
  'Зустріч',
  'Таємниця',
  'Свято',
  'Бійка',
  'Вбивство',
  'Весілля',
  'Дружба',
  'Знайомство',
  'Любов',
  'Небезпека',
  'Незнайомі голоси',
  'Несподівана зустріч',
  'Переміщення у часі',
  'Побачення',
  'Пожежа',
  'Похорон',
  'Розлука',
  'Сварка',
  'Смерть',
  'Флірт',
  'Мрія',
  'Народження',
  'Хвороба',
  'Одужання',
  'Зрада',
  'Прощення',
  'Помста',
  'Відкриття',
  'Винахід',
  'Подорож',
  'Втрата пам\'яті',
  'Викрадення',
  'Пограбування',
  'Арешт',
  'Втеча з в\'язниці',
  'Повстання',
  'Революція',
  'Переворот',
  'Голод',
  'Посуха',
  'Повінь',
  'Землетрус',
  'Шторм',
  'Аварія',
  'Катастрофа',
  'Епідемія',
  'Карантин',
  'Ізоляція',
  'Самотність',
  'Зцілення',
  'Прозріння',
  'Спокуса',
  'Покаяння',
  'Жертвопринесення',
  'Ритуал',
  'Ініціація',
  'Випускний',
  'День народження',
  'Ювілей',
  'Розлучення',
  'Возз\'єднання',
  'Примирення',
  'Зникнення',
  'Пошук',
  'Переслідування',
  'Гонитва',
  'Засідка',
  'Дуель',
  'Битва',
  'Перемога',
  'Поразка',
  'Капітуляція',
  'Звільнення',
  'Втрата',
  'Знахідка',
  'Викриття',
  'Обман',
  'Шантаж',
  'Погроза',
  'Крадіжка',
  'Афера',
  'Змова',
  'Ревнощі',
  'Заздрість',
  'Розчарування',
  'Натхнення',
  'Успіх',
  'Провал',
  'Випробування',
  'Ризик',
  'Порятунок',
  'Диво',
  'Пророцтво',
  'Прокляття',
  'Благословення',
  'Прощання',
  'Еміграція',
  'Переїзд',
  'Усиновлення'
];

const mainCharacters = [
  'Поліцейський',
  'Учень',
  'Інопланетянин',
  'Шпигун',
  'Алкаш',
  'Ангел',
  'Ангел-охоронець',
  'Бабуся',
  'Бомж',
  'Дівчина',
  'Дідусь',
  'Жінка',
  'Іноземець',
  'Злочинець',
  'Зомбі',
  'Казкова істота',
  'Коханка',
  'Коханець',
  'Лікар',
  'Маніяк',
  'Маленька дівчинка',
  'Маленький хлопчик',
  'Мудрець',
  'Невиліковно хворий',
  'Письменник',
  'Привид',
  'Самогубець',
  'Священник',
  'Солдат',
  'Чарівник',
  'Чоловік',
  'Чорт',
  'Хлопець',
  'Дідусь (як родич)',
  'Бабуся (як родич)',
  'Дружина',
  'Чоловік (як родич)',
  'Син',
  'Донька',
  'Двійник',
  'Друг',
  'Сусід',
  'Вчитель',
  'Студент',
  'Ветеран',
  'Сирота',
  'Мачуха',
  'Вітчим',
  'Тітка',
  'Дядько',
  'Онук',
  'Онука',
  'Наречений',
  'Наречена',
  'Колега',
  'Начальник',
  'Підлеглий',
  'Вбивця',
  'Детектив',
  'Суддя',
  'Адвокат',
  'Медсестра',
  'Хірург',
  'Психіатр',
  'Психолог',
  'Художник',
  'Музикант',
  'Актор',
  'Актриса',
  'Співак',
  'Танцівниця',
  'Бізнесмен',
  'Політик',
  'Президент',
  'Король',
  'Королева',
  'Принц',
  'Принцеса',
  'Лицар',
  'Воїн',
  'Розбійник',
  'Пірат',
  'Русалка',
  'Вампір',
  'Перевертень',
  'Демон',
  'Бог',
  'Богиня',
  'Дух',
  'Відьма',
  'Екстрасенс',
  'Мандрівник',
  'Турист',
  'Продавець',
  'Офіціант',
  'Водій',
  'Пілот',
  'Няня',
  'Робот',
  'Штучний інтелект'
];

const secondaryCharacters = [
  'Поліцейський',
  'Учень',
  'Інопланетянин',
  'Шпигун',
  'Алкаш',
  'Ангел',
  'Ангел-охоронець',
  'Бабуся',
  'Бомж',
  'Дівчина',
  'Дідусь',
  'Жінка',
  'Іноземець',
  'Злочинець',
  'Зомбі',
  'Казкова істота',
  'Коханка',
  'Коханець',
  'Лікар',
  'Маніяк',
  'Маленька дівчинка',
  'Маленький хлопчик',
  'Мудрець',
  'Невиліковно хворий',
  'Письменник',
  'Привид',
  'Самогубець',
  'Священник',
  'Солдат',
  'Чарівник',
  'Чоловік',
  'Чорт',
  'Хлопець',
  'Дідусь (як родич)',
  'Бабуся (як родич)',
  'Дружина',
  'Чоловік (як родич)',
  'Син',
  'Донька',
  'Двійник',
  'Друг',
  'Сусід',
  'Вчитель',
  'Студент',
  'Ветеран',
  'Сирота',
  'Мачуха',
  'Вітчим',
  'Тітка',
  'Дядько',
  'Онук',
  'Онука',
  'Наречений',
  'Наречена',
  'Колега',
  'Начальник',
  'Підлеглий',
  'Вбивця',
  'Детектив',
  'Суддя',
  'Адвокат',
  'Медсестра',
  'Хірург',
  'Психіатр',
  'Психолог',
  'Художник',
  'Музикант',
  'Актор',
  'Актриса',
  'Співак',
  'Танцівниця',
  'Бізнесмен',
  'Політик',
  'Президент',
  'Король',
  'Королева',
  'Принц',
  'Принцеса',
  'Лицар',
  'Воїн',
  'Розбійник',
  'Пірат',
  'Русалка',
  'Вампір',
  'Перевертень',
  'Демон',
  'Бог',
  'Богиня',
  'Дух',
  'Відьма',
  'Екстрасенс',
  'Мандрівник',
  'Турист',
  'Продавець',
  'Офіціант',
  'Водій',
  'Пілот',
  'Няня',
  'Робот',
  'Штучний інтелект'
];

const victims = [
  'Поліцейський', 'Учень', 'Інопланетянин', 'Шпигун', 'Алкаш', 'Ангел', 'Ангел-охоронець', 'Бабуся', 'Бомж', 'Дівчина', 'Дідусь', 'Жінка', 'Іноземець', 'Злочинець', 'Зомбі', 'Казкова істота', 'Коханка', 'Коханець', 'Лікар', 'Маніяк', 'Маленька дівчинка', 'Маленький хлопчик', 'Мудрець', 'Невиліковно хворий', 'Письменник', 'Привид', 'Самогубець', 'Священник', 'Солдат', 'Чарівник', 'Чоловік', 'Чорт', 'Хлопець', 'Дідусь (як родич)', 'Бабуся (як родич)',
  'Дружина',
  'Чоловік (як родич)',
  'Син',
  'Донька',
  'Двійник',
  'Друг',
  'Сусід',
  'Вчитель',
  'Студент',
  'Ветеран',
  'Сирота',
  'Мачуха',
  'Вітчим',
  'Тітка',
  'Дядько',
  'Онук',
  'Онука',
  'Наречений',
  'Наречена',
  'Колега',
  'Начальник',
  'Підлеглий',
  'Вбивця',
  'Детектив',
  'Суддя',
  'Адвокат',
  'Медсестра',
  'Хірург',
  'Психіатр',
  'Психолог',
  'Художник',
  'Музикант',
  'Актор',
  'Актриса',
  'Співак',
  'Танцівниця',
  'Бізнесмен',
  'Політик',
  'Президент',
  'Король',
  'Королева',
  'Принц',
  'Принцеса',
  'Лицар',
  'Воїн',
  'Розбійник',
  'Пірат',
  'Русалка',
  'Вампір',
  'Перевертень',
  'Демон',
  'Бог',
  'Богиня',
  'Дух',
  'Відьма',
  'Екстрасенс',
  'Мандрівник',
  'Турист',
  'Продавець',
  'Офіціант',
  'Водій',
  'Пілот',
  'Няня',
  'Робот',
  'Штучний інтелект'
];

const suspects = [
  'Поліцейський',
  'Учень',
  'Інопланетянин',
  'Шпигун',
  'Алкаш',
  'Ангел',
  'Ангел-охоронець',
  'Бабуся',
  'Бомж',
  'Дівчина',
  'Дідусь',
  'Жінка',
  'Іноземець',
  'Злочинець',
  'Зомбі',
  'Казкова істота',
  'Коханка',
  'Коханець',
  'Лікар',
  'Маніяк',
  'Маленька дівчинка',
  'Маленький хлопчик',
  'Мудрець',
  'Невиліковно хворий',
  'Письменник',
  'Привид',
  'Самогубець',
  'Священник',
  'Солдат',
  'Чарівник',
  'Чоловік',
  'Чорт',
  'Хлопець',
  'Дідусь (як родич)',
  'Бабуся (як родич)',
  'Дружина',
  'Чоловік (як родич)',
  'Син',
  'Донька',
  'Двійник',
  'Друг',
  'Сусід',
  'Вчитель',
  'Студент',
  'Ветеран',
  'Сирота',
  'Мачуха',
  'Вітчим',
  'Тітка',
  'Дядько',
  'Онук',
  'Онука',
  'Наречений',
  'Наречена',
  'Колега',
  'Начальник',
  'Підлеглий',
  'Вбивця',
  'Детектив',
  'Суддя',
  'Адвокат',
  'Медсестра',
  'Хірург',
  'Психіатр',
  'Психолог',
  'Художник',
  'Музикант',
  'Актор',
  'Актриса',
  'Співак',
  'Танцівниця',
  'Бізнесмен',
  'Політик',
  'Президент',
  'Король',
  'Королева',
  'Принц',
  'Принцеса',
  'Лицар',
  'Воїн',
  'Розбійник',
  'Пірат',
  'Русалка',
  'Вампір',
  'Перевертень',
  'Демон',
  'Бог',
  'Богиня',
  'Дух',
  'Відьма',
  'Екстрасенс',
  'Мандрівник',
  'Турист',
  'Продавець',
  'Офіціант',
  'Водій',
  'Пілот',
  'Няня',
  'Робот',
  'Штучний інтелект'
];

// Функція для вибору випадкового елемента
function getRandomItem(array) {
    const index = Math.floor(Math.random() * array.length);
    return array[index];
}

// Функція для показу результату
function showResult(buttonId, array) {
    const resultDiv = document.getElementById(`result${buttonId}`);
    resultDiv.textContent = getRandomItem(array); // Вибираємо випадковий елемент
    resultDiv.style.display = 'block'; // Показуємо результат
}

// Обробники подій на кнопки
document.getElementById('actEmotion').addEventListener('click', function () {
    showResult('Emotion', genres); // Покажемо результат для "Жанр"
});

document.getElementById('actBodyParts').addEventListener('click', function () {
    showResult('BodyParts', locations); // Покажемо результат для "Локація"
});

document.getElementById('actImprov').addEventListener('click', function () {
    showResult('Improv', situations); // Покажемо результат для "Ситуація"
});

document.getElementById('actMainCharacter').addEventListener('click', function () {
    showResult('MainCharacter', mainCharacters); // Покажемо результат для "Головний герой"
});

document.getElementById('actLetters').addEventListener('click', function () {
    showResult('Letters', secondaryCharacters); // Покажемо результат для "Другорядний персонаж"
});

document.getElementById('victim').addEventListener('click', function () {
    showResult('Victims', victims); // Покажемо результат для "Жертва"
});

document.getElementById('suspect').addEventListener('click', function () {
    showResult('Suspect', suspects); // Покажемо результат для "Підозрюваний"
});
